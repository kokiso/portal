      ...
      if($retCls["retorno"] != "OK"){
        ...
        if( $rotina=="selectNo" ){
          $sql="SELECT NO_CODIGO
                       ,NO_NOME
                       ,CASE WHEN A.NO_FINNFE='1' THEN CAST('NFe_NORMAL' AS VARCHAR(10))
                             WHEN A.NO_FINNFE='2' THEN CAST('NFe_COMPLEM' AS VARCHAR(11))
                             WHEN A.NO_FINNFE='3' THEN CAST('NFe_AJUSTE' AS VARCHAR(10))
                             WHEN A.NO_FINNFE='4' THEN CAST('DEVOL_RETORNO' AS VARCHAR(13)) END AS NO_FINNFE
                       ...
                       ,US_APELIDO
                       ,NO_CODUSR
                  FROM NATUREZAOPERACAO A
                  LEFT OUTER JOIN USUARIOSISTEMA U ON A.NO_CODUSR=U.US_CODIGO
                 WHERE (NO_ATIVO='".$lote[0]->ativo."') OR ('*'='".$lote[0]->ativo."')";                 
...
<!DOCTYPE html>
  <head>
    ...
    <script>
      ...
      //
      var objNo;                      // Obrigatório para instanciar o JS TFormaCob
