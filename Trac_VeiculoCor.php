<?php
  ...
  if( isset($_POST["veiculocor"]) ){
    try{     
      ...
      $vldr     = new validaJSon();          
      $retorno  = "";
      $retCls   = $vldr->validarJs($_POST["veiculocor"]);
      ///////////////////////////////////////////////////////////////////////
      ...
      if($retCls["retorno"] != "OK"){
        ...
        /////////////////////////////////////////////
        if( $rotina=="selectVeiculoCor" ){
          $sql="";
          $sql.="SELECT A.VCR_CODIGO";
          $sql.="       ,A.VCR_NOME";
          $sql.="       ,CASE WHEN A.VCR_ATIVO='S' THEN 'SIM' ELSE 'NAO' END AS VCR_ATIVO";
          $sql.="       ,CASE WHEN A.VCR_REG='P' THEN 'PUB' WHEN A.VCR_REG='S' THEN 'SIS' ELSE 'ADM' END AS VCR_REG";
          $sql.="       ,U.US_APELIDO";
          $sql.="       ,A.VCR_CODUSR";
          $sql.="  FROM VEICULOCOR A";
          $sql.="  LEFT OUTER JOIN USUARIOSISTEMA U ON A.VCR_CODUSR=U.US_CODIGO";
          $sql.="  WHERE (VCR_ATIVO='".$lote[0]->ativo."') OR ('*'='".$lote[0]->ativo."')";
        ...
        if( $rotina=="impExcel" ){
          ...
          $vldCampo = new validaCampo("VVEICULOCOR",0);
          //////////////////////////////////////////////////////////////////////////////////////
  ...
  <head>
    ...
    <script>
      ...
      document.addEventListener("DOMContentLoaded", function(){ 
        //////////////////////////////////////////
        //   Objeto clsTable2017 VEICULOCOR     //
        //////////////////////////////////////////
        jsVcr={
          "titulo":[
             {"id":0  ,"labelCol":"OPC"     
                      ,"padrao":1}            
            ,{"id":1  ,"field"          :"VCR_CODIGO" 
                      ,"labelCol"       : "CODIGO"
                      ,"obj"            : "edtCodigo"
                      ,"tamGrd"         : "5em"
                      ,"tamImp"         : "15"
          ...
          ,"div"            : "frmVcr"              // Onde vai ser gerado a table
          ,"divFieldSet"    : "tabelaVcr"           // Para fechar a div onde estão os fieldset ao cadastrar
          ,"form"           : "frmVcr"              // Onde vai ser gerado o fieldSet       
          ,"divModal"       : "divTopoInicio"       // Onde vai se appendado abaixo deste a table 
          ,"tbl"            : "tblVcr"              // Nome da table
          ,"prefixo"        : "vcr"                 // Prefixo para elementos do HTML em jsTable2017.js
          ,"tabelaBD"       : "VVEICULOCOR"         // Nome da tabela no banco de dados  
          ,"tabelaBKP"      : "BKPVEICULOCOR"       // Nome da tabela no banco de dados  
          ,"fieldAtivo"     : "VCR_ATIVO"           // SE EXISITIR - Nome do campo ATIVO(S/N) na tabela BD
          ,"fieldReg"       : "VCR_REG"             // SE EXISITIR - Nome do campo SYS(P/A) na tabela BD            
          ,"fieldCodUsu"    : "VCR_CODUSR"          // SE EXISITIR - Nome do campo CODIGO USUARIO na tabela BD                        
          ,"iFrame"         : "iframeCorpo"         // Se a table vai ficar dentro de uma tag iFrame
          ,"width"          : "73em"                // Tamanho da table
          ,"height"         : "58em"                // Altura da table
          ,"tableLeft"      : "sim"                 // Se tiver menu esquerdo
          ,"relTitulo"      : "COR"                 // Titulo do relatório
          ...
          ,"_menuTable"     :[
                               ...
                               ,["Modelo planilha excel"                  ,"fa-file-excel-o"  ,"objVcr.modeloExcel()"]
                               ,["Ajuda para campos padrões"              ,"fa-info"          ,"objVcr.AjudaSisAtivo(jsVcr);"]
                               ,["Detalhe do registro"                    ,"fa-folder-o"      ,"objVcr.detalhe();"]
                               ,["Passo a passo do registro"              ,"fa-binoculars"    ,"objVcr.espiao();"]
                               ,["Alterar status Ativo/Inativo"           ,"fa-share"         ,"objVcr.altAtivo(intCodDir);"]
                               ,["Alterar registro PUBlico/ADMinistrador" ,"fa-reply"         ,"objVcr.altPubAdm(intCodDir,jsPub[0].usr_admpub);"]
                               ,["Alterar para registro do SISTEMA"       ,"fa-reply"         ,"objVcr.altRegSistema("+jsPub[0].usr_d31+");"]
                               ,["Número de registros em tela"            ,"fa-info"          ,"objVcr.numRegistros();"]                               
                               ,["Atualizar grade consulta"               ,"fa-filter"        ,"btnFiltrarClick('S');"]                               
                             ]  
          ,"codTblUsu"      : "VEICULOCOR[38]"                          
          ,"codDir"         : intCodDir
        }; 
        if( objVcr === undefined ){  
          objVcr=new clsTable2017("objVcr");
        };  
        objVcr.montarHtmlCE2017(jsVcr); 
        //////////////////////////////////
        // Definindo o form de manutencaum
        //////////////////////////////////    
        document.getElementById(jsVcr.form).style.width=jsVcr.width;
        //
        //
        ////////////////////////////////////////////////////////
      ...
      //
      var objVcr;                     // Obrigatório para instanciar o JS TFormaCob
      var jsVcr;                      // Obj principal da classe clsTable2017
      ...
      function btnFiltrarClick(atv) { 
        clsJs   = jsString("lote");  
        clsJs.add("rotina"      , "selectVeiculoCor"                          );
        ...
        if( retPhp[0].retorno == "OK" ){
          ...
          //////////////////////////////////////////////////////////////////////////////////
          jsVcr.registros=objVcr.addIdUnico(retPhp[0]["dados"]);
          objVcr.ordenaJSon(jsVcr.indiceTable,false);  
          objVcr.montarBody2017();
        }; 
      };
      ////////////////////
      // Importar excel //
      ...
      function btnAbrirExcelClick(){
        try{
          ...
          if( clsErro.ListaErr() != "" ){
            ...
            clsJs.add("login"       , jsPub[0].usr_login          );
            clsJs.add("titulo"      , objVcr.trazCampoExcel(jsVcr));    
            envPhp=clsJs.fim();  
            fd = new FormData();
            fd.append("veiculocor"      , envPhp              );
            ...
            /////////////////////////////////////////////////////////////////////////////////////////
            gerarMensagemErro("Cor",retPhp[0].erro,{cabec:"Aviso"});    
          };  
        } catch(e){
    ...
    <div class="divTelaCheia">
      <div id="divRotina" class="conteudo">  
        ...
        <form method="post" 
              ...
              target="_newpage">
          <div class="frmTituloManutencao">Cor<img class="frmTituloManutencaoImg" src="imagens\chave.png" title="campo obrigatório" /></div>              
