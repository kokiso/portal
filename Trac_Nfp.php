    ...
    try{     
      ...
      if($retCls["retorno"] != "OK"){
        ...
        if( $lote[0]->rotina=="cancelanf" ){
          $sql="UPDATE NFPRODUTO SET NFP_DTCANCELA='".$lote[0]->data."',NFP_CODUSR=".$_SESSION["usr_codigo"]." WHERE NFP_LANCTO=".$lote[0]->lancto;
        ...
        if( $lote[0]->rotina=="regsistema" ){
          ...
          foreach ( $objReg as $reg ){
            $sql="UPDATE NFPRODUTO SET NFP_REG='S',NFP_CODUSR=".$_SESSION["usr_codigo"]." WHERE NFP_LANCTO=".$reg->lancto;
            array_push($arrUpdt,$sql);            
          };
          $atuBd = true;
        };
        ...
        if( $lote[0]->rotina=="filtrar" ){        
          $sql ="SELECT A.NFP_LANCTO AS LANCTO";
          $sql.="       ,A.NFP_CODCMP AS CODCMP";
          $sql.="       ,CMP.CMP_NOME AS COMPET";          
          $sql.="       ,A.NFP_NUMNF AS NF";          
          ...
          $sql.="       ,A.NFP_VLRTOTAL AS VALOR";
          $sql.="       ,CAST(0 AS NUMERIC(15,2)) AS RETENCAO";
          $sql.="       ,SNF.SNF_LIVRO AS LIVRO";
          $sql.="       ,SNF.SNF_CODFLL AS FILIAL";
          $sql.="       ,CONVERT(VARCHAR(10),A.NFP_DTCANCELA,127) AS CANCELADA";                    
          $sql.="       ,CASE WHEN SNF.SNF_ENVIO='P' THEN 'SIM' ELSE 'NAO' END AS ENVIO";                    
          $sql.="       ,A.NFP_RECIBONFE AS RECIBO";          
          $sql.="       ,CASE WHEN A.NFP_REG='P' THEN 'PUB' WHEN A.NFP_REG='S' THEN 'SIS' ELSE 'ADM' END AS REG";          
          $sql.="       ,US.US_APELIDO";          
          $sql.="  FROM NFPRODUTO A WITH(NOLOCK)";           
          $sql.="  LEFT OUTER JOIN PAGAR PGR ON A.NFP_LANCTO=PGR.PGR_LANCTO";          
          $sql.="  LEFT OUTER JOIN FAVORECIDO FVR ON PGR.PGR_CODFVR=FVR.FVR_CODIGO";          
          $sql.="  LEFT OUTER JOIN SERIENF SNF ON A.NFP_CODSNF=SNF.SNF_CODIGO";          
          $sql.="  LEFT OUTER JOIN COMPETENCIA CMP ON A.NFP_CODCMP=CMP.CMP_CODIGO AND CMP.CMP_CODEMP=SNF.SNF_CODEMP";                    
          $sql.="  LEFT OUTER JOIN USUARIOSISTEMA US ON US.US_CODIGO = A.NFP_CODUSR";          
          $sql.=" WHERE (A.NFP_CODCMP='".$lote[0]->codcmp."')";
  ...
  <head>
    ...
    <script>      
      ...
      document.addEventListener("DOMContentLoaded", function(){ 
        ...
        jsNfp={
          "titulo":[
            ...
            ,{"id":15 ,"labelCol"       : "NFe"
                      ...
                      ,"excel"          : "S"
                      ,"ordenaColuna"   : "S"
                      ,"popoverTitle"   : "Parametro informando se NF deve ser enviada ao SEFAZ(NF Eletronica)"                          
                      ,"popoverLabelCol": "NFe"                      
                      ,"padrao":0}
          ...
          ,"div"            : "frmNfp"                  // Onde vai ser gerado a table
          ,"divFieldSet"    : "tabelaNfp"               // Para fechar a div onde estão os fieldset ao cadastrar
          ,"form"           : "frmNfp"                  // Onde vai ser gerado o fieldSet       
          ,"divModal"       : "divTopoInicio"           // Onde vai se appendado abaixo deste a table 
          ,"divModalDentro" : "sctnNfp"                 // Onde vai se appendado dentro do objeto(Ou uma ou outra, se existir esta despreza a divModal)
          ,"tbl"            : "tblNfp"                  // Nome da table
          ,"prefixo"        : "Me"                      // Prefixo para elementos do HTML em jsTable2017.js
          ,"tabelaBD"       : "VNFPRODUTO"              // Nome da tabela no banco de dados  
          ...
          ,"relTitulo"      : "NFP"                     // Titulo do relatório
        ...
        if( objNfp === undefined ){  
          objNfp=new clsTable2017("objNfp");
        };  
        objNfp.montarHtmlCE2017(jsNfp); 
        //////////////////////////////////////////////////////////////////////
        // Aqui sao as colunas que vou precisar aqui e Trac_GrupoModeloInd.php
        // esta garante o chkds[0].?????? e objCol
        //////////////////////////////////////////////////////////////////////
        objCol=fncColObrigatoria.call(jsNfp,["CANCELADA","LANCTO","REG","USUARIO"]);
        btnFiltrarClick();
      });
      var objNfp;                     // Obrigatório para instanciar o JS TFormaCob
      var jsNfp;                      // Obj principal da classe clsTable2017
      ...
      function btnFiltrarClick() { 
        ...
        if( retPhp[0].retorno == "OK" ){
          ...
          //////////////////////////////////////////////////////////////////////////////////
          jsNfp.registros=objNfp.addIdUnico(retPhp[0]["dados"]);
          objNfp.ordenaJSon(jsNfp.indiceTable,false);  
          objNfp.montarBody2017();
        };  
      ...
      function horNovaNfClick(){
        window.open("Trac_NfpCadTitulo.php");
      };
      ...
      function horCancelaNfClick(){
        try{
          clsChecados = objNfp.gerarJson("1");
          ...
          if( retPhp[0].retorno=="OK" ){
            ...
            // Atualizando a grade
            /////////////////////////////////////////////////
            let tbl = tblNfp.getElementsByTagName("tbody")[0];
      ...
      function fncRegSistema(){
        try{    
          clsChecados = objNfp.gerarJson("n");
          chkds       = clsChecados.gerar();
          /////////////////////////////////////////////////
        ...
        <div class="colMd12" style="float:left;margin-bottom:0px;height:50px;">
          <div class="infoBox">
            ...
            <div class="infoBoxContent">
              <span class="infoBoxText">NF Produto</span>
