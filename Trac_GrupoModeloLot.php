<?php
  ...
  if( isset($_POST["grupomodelolot"]) ){
    try{     
      ...
      $vldr     = new validaJSon();          
      $retorno  = "";
      $retCls   = $vldr->validarJs($_POST["grupomodelolot"]);
      ///////////////////////////////////////////////////////////////////////
      ...
      if($retCls["retorno"] != "OK"){
        ...
        if( $lote[0]->rotina=="exclote" ){
          ...
          $erro ="ok";
          $sql  ="";
          $sql.="SELECT GMP_CODIGO,GMP_CODPE FROM GRUPOMODELOPRODUTO";          
          $sql.=" WHERE ((GMP_CODIGO BETWEEN ".$lote[0]->codini." AND ".$lote[0]->codfim.") AND (GMP_CODGML=".$lote[0]->lote."))";
          $classe->msgSelect(true);
          $retCls = $classe->selectAssoc($sql);
          $tbl    = $retCls["dados"];
          ///////////////////////////////////////////////////////////////////////////////
          // Para excluir um lote o produto deve estar no ponto de estoque "EST[Estoque]"
          // A variavel $contador tem que ser o campo GML_ESTOQUE( A qtdade que entrou )
          ///////////////////////////////////////////////////////////////////////////////
          foreach ( $tbl as $reg ){
            if( $reg["GMP_CODPE"]=="EST" ){
              $sql  ="DELETE FROM VGRUPOMODELOPRODUTO WHERE GMP_CODIGO=".$reg["GMP_CODIGO"];  
              array_push($arrUpdt,$sql);            
              $contador++;
              $entrada=$lote[0]->entrada;
            } else {
              $erro=" PRODUTO ".$reg["GMP_CODIGO"]." DEVE ESTAR NO PONTO DE ESTOQUE [EST]"; 
            };
          };  
          if( $erro=="ok" ){
            if( $contador != $entrada ){
          ...
          ////////////////////
          if( $erro=="ok" ){          
            $sql="DELETE FROM GRUPOMODELOLOTE WHERE GML_CODIGO=".$lote[0]->lote;
            array_push($arrUpdt,$sql);            
            $atuBd = true;
          };
        };  
...
<!DOCTYPE html>
    ...
    <script>      
      ...
      document.addEventListener("DOMContentLoaded", function(){
        ...
        jsGml={
          ...
          , 
          "botoesH":[
             {"texto":"Excluir lote"  ,"name":"horExcLot"     ,"onClick":"7"  ,"enabled":true ,"imagem":"fa fa-minus"       }          
          ...
          ,"div"            : "frmGml"                  // Onde vai ser gerado a table
          ,"divFieldSet"    : "tabelaGml"               // Para fechar a div onde estão os fieldset ao cadastrar
          ,"form"           : "frmGml"                  // Onde vai ser gerado o fieldSet       
          ,"divModal"       : "divTopoInicio"           // Onde vai se appendado abaixo deste a table 
          ,"divModalDentro" : "sctnGml"                 // Onde vai se appendado dentro do objeto(Ou uma ou outra, se existir esta despreza a divModal)
          ,"tbl"            : "tblGml"                  // Nome da table
          ,"prefixo"        : "Gml"                     // Prefixo para elementos do HTML em jsTable2017.js
        ...
        if( objGml === undefined ){  
          objGml=new clsTable2017("objGml");
        };  
        objGml.montarHtmlCE2017(jsGml);
        //////////////////////////////////////////////////////////////////////
        // Aqui sao as colunas que vou precisar aqui e Trac_GrupoModeloInd.php
        // esta garante o chkds[0].?????? e objCol
        //////////////////////////////////////////////////////////////////////
        objCol=fncColObrigatoria.call(jsGml,["ENTRADA","LOTE","INI","FIM"]);
      });
      //
      var objGml;                     // Obrigatório para instanciar o JS Semestral
      var jsGml;                      // Obj principal da classe clsTable2017
      ...
      function btnFiltrarClick() { 
        ...
        fd = new FormData();
        fd.append("grupomodelolot" , clsJs.fim());
        msg     = requestPedido("Trac_GrupoModeloLot.php",fd); 
        ...
        if( retPhp[0].retorno == "OK" ){
          ...
          //////////////////////////////////////////////////////////////////////////////////
          jsGml.registros=objGml.addIdUnico(retPhp[0]["dados"]);
          objGml.ordenaJSon(jsGml.indiceTable,false);  
          objGml.montarBody2017();
        };  
      };
      //////////////////////
      // Exclusao de um lote
      ...
      function horExcLotClick(){
        try{
          clsChecados = objGml.gerarJson("1");
          ...
          if( retPhp[0].retorno=="OK" ){
            ...
            for(let lin=0 ; (lin<nl) ; lin++){
              if( jsNmrs(chkds[0].CODGM).inteiro().ret() == jsNmrs(tbl.rows[lin].cells[parseInt(pega.colCodigo)].innerHTML).inteiro().ret() ){
  ...
  <body style="background-color: #ecf0f5;">
    <div class="divTelaCheia">
      ...
      <section>
        <section id="sctnGml" style="margin-left:100px;">
        </section>  
      </section>
      <form method="post" class="center" id="frmGml" action="classPhp/imprimirSql.php" target="_newpage" style="margin-top:1em;" >
